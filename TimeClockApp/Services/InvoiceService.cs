	...
	public class InvoiceService : ReportPageService
	{
...
#if WINDOWS
		[System.Diagnostics.CodeAnalysis.DynamicDependency(System.Diagnostics.CodeAnalysis.DynamicallyAccessedMemberTypes.All, typeof(Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions))]
#endif
		public async Task<List<TimeSheet>> RunInvoiceReportAsync(bool usePhaseFilter, Project? project, Phase? phase, DateOnly? start, DateOnly? end)
		{
			...
			foreach (var emp in e)
			{
				...
				{
					List<TimeCard> validCards = g.Where(x => x.TimeCard_Status is ShiftStatus.ClockedOut or ShiftStatus.Paid).ToList();
